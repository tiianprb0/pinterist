...
<head>
    ...
    <style>
        ...
        .pin-detail-content .pin-grid {
            /* display: grid; Hapus ini sesuai permintaan */
...
<body>
    ...
    <div id="pinDetailOverlay">
        <div class="pin-detail-content">
            ...
            <div id="pinDetailPersonTagsSection" class="pin-person-tags-section" style="display: none;">
                ...
                <!-- Menghapus inline style="display: grid;" -->
    ...
    <script>
        ...
        let currentSearchQuery = '<?php echo htmlspecialchars(str_replace('-', ' ', $personName)); ?>';
        let allPinsData = [];
        ...
        function getCorrectedImagePath(originalPath) {
            // Asumsi who.php berada di Spicette/
            ...
            if (originalPath.startsWith('./uploads/pins/')) {
                return originalPath.substring(2); // Hapus './'
            }
            // Jika path sudah dimulai dengan 'uploads/pins/', biarkan saja
            if (originalPath.startsWith('uploads/pins/')) {
                return originalPath;
            }
        ...
        async function openPinDetail(pinData) {
            ...
            if (pinData.images && pinData.images.length > 0) {
                if (pinData.display_type === 'slider' && pinData.images.length > 1) {
                    ...
                    pinData.images.forEach((image, index) => {
                        ...
                        // Gunakan getCorrectedImagePath untuk gambar di slider
                    ...
                    setupTouchSlider('pinSlider');
                } else {
                    pinData.images.forEach(image => {
                        const imgDiv = document.createElement('div');
                        imgDiv.className = 'stacked-image-item';
                        const imgElement = document.createElement('img');
                        // Gunakan getCorrectedImagePath untuk gambar bertumpuk
                        imgElement.src = getCorrectedImagePath(image.url);
            ...
            if (pinData.personTags && pinData.personTags.length > 0) {
                ...
                for (const person of taggedPeople) {
                    ...
                    // Pastikan tautan ke who.php menggunakan path absolut yang benar
                    ...
                    // Pastikan tautan "Lihat Semua" ke who.php menggunakan path absolut yang benar
                ...
                if (finalShuffledRelatedPins.length > 0) {
                    ...
                    // Tidak perlu mengatur style.display: grid; di sini karena sudah dihapus dari CSS
                } else {
            ...
            if (pinData.categories && Array.isArray(pinData.categories) && pinData.categories.length > 0) { 
                pinData.categories.forEach(categoryName => {
                    ...
                    // Menggunakan window.location.origin untuk path absolut yang benar
        ...
        function createPinElement(pinData) {
            ...
            // Gunakan getCorrectedImagePath untuk gambar utama di grid
            ...
            if (pinData.categories && Array.isArray(pinData.categories) && pinData.categories.length > 0) { 
                 ...
                 // Gunakan window.location.origin untuk path absolut yang benar
        ...
        async function loadPins(count, append = true) {
            ...
            if (currentSearchQuery) { 
                endpoint = 'pins.php?action=getPinsByPersonTag';
                params = `&name=${encodeURIComponent(currentSearchQuery)}`;
            } else { 
        ...
        if (mobileDropdownLogout) { 
            mobileDropdownLogout.addEventListener('click', async () => {
                ...
                if (response.success) {
                    currentUser = null;
                    updateProfileDisplay();
                    // updateHeaderDropdown(); // Dihapus karena tidak ada di who.php
                    showMessage('Logout berhasil.', 'Logout');
                    loadedPinsCount = 0;
                    currentSearchQuery = ''; 
                    await loadPins(pinsPerPage, false);
                    ...
                    const homeMobileNav = document.querySelector('.mobile-nav-item[data-action="home"]');
                    if (homeMobileNav) homeMobileNav.classList.add('active');
                    // headerNavButtons.forEach(btn => btn.classList.remove('active')); // Dihapus karena tidak ada di who.php
                    // const homeHeaderNav = document.querySelector('.nav-button[data-nav="home"]'); // Dihapus karena tidak ada di who.php
                    // if (homeHeaderNav) homeHeaderNav.classList.add('active'); // Dihapus karena tidak ada di who.php
                } else {
                    showMessage('Logout gagal: ' + response.message, 'Error');
                }
                if (mobileProfileDropdown) { mobileProfileDropdown.style.display = 'none'; }
