    ...
    <script>
        ...
        function openPinDetail(pinData) {
            ...
            if (pinData.category) {
                ...
                span.innerHTML = `<a href="tag/${encodeURIComponent(categoryPermalink)}" style="color: inherit; text-decoration: none;">#${pinData.category.trim()}</a>`;
        ...
        // Function to add swipe support to the slider, allowing clicks for taps
        function setupTouchSlider(sliderId) {
            ...
            let touchStartX = 0;
            let touchEndX = 0;
            let isDragging = false; // Flag to indicate if a drag/swipe is happening
            const minMovementThreshold = 10; // Minimum pixel movement to consider it a drag, not just a tap
            const minSwipeDistance = 50; // Minimum distance for a swipe to register a slide change

            slider.addEventListener('touchstart', (e) => {
                touchStartX = e.changedTouches[0].screenX;
                isDragging = false; // Reset on touchstart
            }, { passive: true });

            slider.addEventListener('touchmove', (e) => {
                // Detect if touch has moved significantly enough to be considered a drag
            ...
            slider.addEventListener('touchend', (e) => {
                ...
                if (isDragging) { // Only handle as swipe if a drag occurred
                    // Only perform slide change if a sufficient swipe distance is met
                    if (touchEndX < touchStartX - minSwipeDistance) {
                        moveSlide(1, sliderId); // Swipe left (next slide)
                    } else if (touchEndX > touchStartX + minSwipeDistance) {
                        moveSlide(-1, sliderId); // Swipe right (previous slide)
                    }
                } else {
                    // If it's not a drag (i.e., a tap), allow default click behavior.
                    // No specific action needed here, as the image's onclick will fire.
                }
            }, { passive: true });
        }
        ...
        window.openFullImageOverlay = function(imageUrl) { // Made global
            console.log('Membuka fullImageOverlay untuk:', imageUrl); // Untuk debugging
            fullImageDisplay.src = imageUrl;
            fullImageDisplay.onerror = function() { // Add onerror for full image
                this.onerror=null; 
        ...
        window.closeFullImageOverlay = function() { // Made global
            console.log('Menutup fullImageOverlay'); // Untuk debugging
        ...
        async function loadPins(count, append = true) {
            ...
            } else { // Removed `else if (currentTab === 'saved')` condition here as it's handled by search query
        ...
        function showSuggestions(inputElement, suggestionsList, query) {
            ...
            ).map(pin => pin.title || pin.category).filter(Boolean).slice(0, 5); // Take title or category
