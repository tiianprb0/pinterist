...
<body>
    ...
    <script>
        ...
        let currentCategoryName = '<?php echo htmlspecialchars(str_replace('-', ' ', $categoryName)); ?>'; // Changed variable name
        ...
        function getCorrectedImagePath(pinImageObject) {
            ...
            if (pinImageObject && pinImageObject.display_url) {
                ...
                // tag.php is in a subfolder, so it needs '../' to go up one level
                if (pinImageObject.display_url.startsWith('./uploads/')) {
                    return '../' + pinImageObject.display_url.substring(2);
                }
        ...
        async function loadPins(count, append = true) {
            ...
            const response = await makeApiRequest(`pins.php?action=search&query=${encodeURIComponent(currentCategoryName)}`); // Corrected endpoint
            ...
            if (response.success) {
                ...
                if (slicedPinsToDisplay.length > 0 || pinsToDisplay.length > 0) { // Check pinsToDisplay.length for initial load
                ...
                if (pinsToDisplay.length === 0) {
                    pinGrid.innerHTML = `<p style="text-align: center; color: #767676; margin-top: 50px;">Tidak ada pin ditemukan untuk kategori "${currentCategoryName}".</p>`;
                }
            } else {
