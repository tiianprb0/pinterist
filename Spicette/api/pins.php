...
if ($method === 'GET') {
    if ($action === 'fetch_all') {
        ...
        $allPins = getAllPins();
        $savedPinIds = readJsonFile($savedPinsDir . $username . '.json');
        ...
        $filteredPins = array_filter($allPins, function($pin) use ($query) {
            ...
            // Search in title, source, and content
    ...
    if ($action === 'add') {
        ...
        foreach ($users as $user) {
            $savedPins = readJsonFile($savedPinsDir . $user['username'] . '.json');
            $updatedSavedPins = array_filter($savedPins, function($id) use ($pinId) {
                return $id !== $pinId;
            });
            writeJsonFile($savedPinsDir . $user['username'] . '.json', array_values($updatedSavedPins));
        }
        ...
        $savedPins = readJsonFile($savedPinsDir . $username . '.json');
        if (!in_array($pinId, $savedPins)) {
            $savedPins[] = $pinId;
            if (writeJsonFile($savedPinsDir . $username . '.json', $savedPins)) {
        ...
        if (writeJsonFile($savedPinsDir . $username . '.json', array_values($updatedSavedPins))) {
